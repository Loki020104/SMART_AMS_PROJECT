...
async function umDeleteSelected(){
  ...
  console.log('[DELETE-BULK] Selected IDs:', idArray);
  if(!confirm(`Delete ${idArray.length} user(s)? This cannot be undone!`)) return;
  
  toast('Deleting users...', 'info');
  
  try{
    // Delete ALL users in parallel for speed
    const deletePromises = idArray.map(userId =>
      fetch(`${window.AMS_CONFIG.API_URL}/api/users/${userId}`, {
        method:'DELETE'
    ...
    console.log('[DELETE-BULK] Results:', results);
    
    if(successCount > 0){
      toast(`✅ Deleted ${successCount} user(s)${failCount>0?` (${failCount} failed)':''}`, 'success');
      window._umSelected.clear();
